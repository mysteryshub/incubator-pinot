...
if [ $noThirdEyeChange -ne 0 ]; then
  ...
  mvn clean install -B -P profile-buildthrift -DskipTests=true -Dmaven.javadoc.skip=true -Dassembly.skipAssembly=true || exit $?
fi
...
if [ $noThirdEyeChange -eq 0 ]; then
  ...
  mvn install -B -DskipTests -P profile-buildthrift -Dmaven.javadoc.skip=true -Dassembly.skipAssembly=true -pl pinot-common,pinot-core,pinot-api -am
